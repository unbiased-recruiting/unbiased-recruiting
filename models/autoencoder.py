...
for df in data:
    ...
    df.loc[:, 'TXT'] = df.loc[:, 'TXT'].apply(
        lambda x: [float(w) / 10000 for w in x])  # Normalizing tokens (10 000 is the maximum and 0 min)
    ...
    for AE_lr in autoencoder_learning_rates:
        ...
        for clf_lr in clf_learning_rates:
            for beta in beta_values:
                ...
                hyparam_name = str(
                    'batchsize_' + str(batch_size) + 'AElr_' + str(AE_lr) + '_CLFlr_' + str(clf_lr) + '_beta_' + str(
                        beta))
                model_params.append(hyparam_name)
                print('(batch_size, AE_learning_rate, clf_learning_rate, beta) = ({} ; {} ; {} ;{})'.format(batch_size,
                                                                                                            AE_lr,
                                                                                                            clf_lr,
                                                                                                            beta))
                ...
                val_clf, val_clf_loss, clf_valid_accuracy, clf_valid_accuracy_op = clf_step(encoder(val_CVs),
                                                                                            val_labels, 'valid')
                val_autoencoder_train, val_adversarial_loss, val_autoencoder_loss = autoencoder_step(val_CVs,
                                                                                                     val_clf_loss, beta)

                valid_running_vars = tf.get_collection(tf.GraphKeys.LOCAL_VARIABLES,
                                                       scope='valid_accuracy')  # to measure validation accuracy during training
                ...
                test_clf, test_clf_loss, test_clf_accuracy, test_clf_accuracy_op = clf_step(encoder(test_CVs),
                                                                                            test_labels, 'test')
                test_autoencoder_train, test_adversarial_loss, test_autoencoder_loss = autoencoder_step(test_CVs,
                                                                                                        test_clf_loss,
                                                                                                        beta)

                test_running_vars = tf.get_collection(tf.GraphKeys.LOCAL_VARIABLES,
                                                      scope='test_accuracy')  # to measure validation accuracy during training
                ...
                with tf.Session() as sess:
                    ...
                    for epoch in range(num_epoch):
                        if epoch % 2 == 0:
                            ...
                            if epoch % FLAGS.info_freq == 0:
                                print("TRAIN epoch: {} , AE loss : {}, ADV loss : {}".format(epoch,
                                                                                             autoencoder_loss_value,
                        ...
                        if (epoch - 1) % FLAGS.info_valid_freq == 0:
                            ...
                            print('VAL epoch: {} , clf accuracy : {}, AE loss : {}, ADV loss : {}'.format(epoch,
                    ...
                    test_results = pd.DataFrame(
                        {'Parameters': model_params, "AE test loss": AE_test_losses, "test_accuracy": test_accuracies,
                         "ADV test loss": ADV_test_losses})  # Overwrite test results
                    test_results.to_csv(os.path.join(saving_path, 'test_results.csv'))

                print('Test results saved')

                # Train results into csv
                print("DEBUG - len of adv, ae loss and acc: ({},{},{})".format(len(adversarial_losses),
                                                                               len(autoencoder_losses),
                                                                               len(clf_accuracies)))
                results = pd.DataFrame(
                    {"train Adversarial loss": adversarial_losses, "train AE loss": autoencoder_losses,
                     "train clf accuracy": clf_accuracies})
                ...
                if len(sys.argv) > 1 and str(sys.argv[1]) == 'plot':
                    ...
                    name = "losses and accuracy vs epochs for Beta={Beta}, batch_size={batch_size}, AE_learning_rate={learning_rate}, classifier_learning_rate={clf_learning_rate}.png".format(
                        Beta=beta, batch_size=batch_size, learning_rate=AE_lr, clf_learning_rate=clf_lr)
                    ...
                    plt.ylabel("Autoencoder Loss")
                    plt.savefig(
                        "graphs/autoencoder loss vs classifier accuracy for Beta={Beta}, batch_size={batch_size}, AE_learning_rate={learning_rate}, classifier_learning_rate={clf_learning_rate}.png".format(
                            Beta=beta, batch_size=batch_size, learning_rate=AE_lr, clf_learning_rate=clf_lr))
