...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_integer('batch_sizes', [30], '')
flags.DEFINE_float('learning_rates', [0.001, 0.01], '')
flags.DEFINE_float('classifier_learning_rates', [0.1, 0.01], '')
flags.DEFINE_float('betas', [10, 1], '')
...
autoencoder_learning_rates = [0.01]  # [0.1,0.01,0.001,0.0001,0.00001]
clf_learning_rates = [0.1]  # [0.0001,0.00001,0.001,0.01, 0.1]
...
for batch_size in FLAGS.batch_sizes:
    for AE_lr in FLAGS.learning_rates:
        ...
        test_iterator = make_iterator(X_test, y_test,
                                      batch_size=batch_size)
        for clf_lr in FLAGS.classifier_learning_rates:
            for beta in beta_values:
                ...
                with tf.Session() as sess:
                    ...
                    for epoch in range(FLAGS.num_epoch):
                        ...
                        if (epoch - 1) % FLAGS.info_valid_freq == 0:
                            ...
                            print('VAL epoch: {} , clf accuracy : {}, AE loss : {}'.format(epoch, valid_accuracy_value,
                                                                                           valid_loss))
                    ...
                    test_results = pd.DataFrame(
                        {'Parameters': model_params, "test loss": test_losses,
                         "test_accuracy": test_accuracies})  # Overwrite test results
                    test_results.to_csv(os.path.join(saving_path, 'AE0.1_test_results.csv'))
                ...
                results = pd.DataFrame(
                    {'train Adversarial loss': adversarial_losses, "train clf accuracy": clf_accuracies})
